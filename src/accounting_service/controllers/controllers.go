...
//	}
func (ctrl *GeneralController[T]) Update(c *gin.Context) {
	// Parse the :id param, but no longer used in logic
	...
	response, err := ctrl.repo.UpdateData(0, &updatedData)
	if err != nil {
		c.JSON(response.Code, gin.H{"code": response.Code, "message": response.Message, "error": err.Error()})
		return
	}
...
func (ctrl *GeneralController[T]) UpsertBatch(c *gin.Context) {
	var dataList []T

	// Parse JSON input as slice
	...
	// Call repository method to handle insert/update batch logic
	response, err := ctrl.repo.UpsertBatch(&dataList)
	if err != nil {
